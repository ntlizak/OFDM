function [ I, Q, kwadr, r1, trans, wyjscie] = QAM32( s, p )

QAM32map= [1+1j, 3+1j, 3+5j, 5+1j, 1+3j, 3+3j, 1+5j, 5+3j, 1-1j, 1-3j, 5-3j, 1-5j, 3-1j, 3-3j, 5-1j, 3-5j, -1+1j, -1+3j, -5+3j, -1+5j, -3+1j, -3+3j, -5+1j, -3+5j, -1-1j, -3-1j, -3-5j, -5-1j, -1-3j, -3-3j, -1-5j, -5-3j];
...
if (mod(dlugosc, 10) ~= 0)
    uzupelnienie = 10 - mod(dlugosc,10);
    temp=s;
...
for i=1:rozmiar(1)
    ...
    temp=sum(temp.*[16 8 4 2 1]);
   
    symbol = QAM32map(temp+1); 
...
Q1=imag(trans);
for i=1:rozmiar
    [a,b] = min(abs((I1(i)+Q1(i)*1j)-QAM32map));
    temp(5)=mod(b-1,2);
    b=floor((b-1)/2);
    temp(4)=mod(b,2);
    b=floor(b/2);
